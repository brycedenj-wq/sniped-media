...
5. Offer + price: os_offer_builder.py build -> os_pricing_gate.py check/options (anchor high).
6. Close: present BEST first; objections -> scope lever not discount.
7. Deliver + log to CRM (os_crm_schema.py). Day-30 -> Op Kit upsell.
HELD: no outreach SENT, no payment setup, no identity exposure without explicit go.
