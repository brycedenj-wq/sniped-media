...
PROJECTS={
 "video_campaign":["HIGGSFIELD_OPERATOR_LIBRARY","PREMIERE_EDITING_LIBRARY","AFTER_EFFECTS_MOTION_LIBRARY","ADOBE_OPERATOR_LIBRARY","FIGMA_DESIGN_SYSTEM_LIBRARY","SOCIAL_DISTRIBUTION_LIBRARY","COMMERCIAL_CRAFT_LIBRARY","STORY_PSYCHOLOGY_LAYER"],
 "film":["HIGGSFIELD_OPERATOR_LIBRARY","PREMIERE_EDITING_LIBRARY","AFTER_EFFECTS_MOTION_LIBRARY","ADOBE_OPERATOR_LIBRARY","BLENDER_PRODUCTION_LIBRARY","COMMERCIAL_CRAFT_LIBRARY","STORY_PSYCHOLOGY_LAYER"],
 "ad":["HIGGSFIELD_OPERATOR_LIBRARY","ADOBE_OPERATOR_LIBRARY","PREMIERE_EDITING_LIBRARY","AFTER_EFFECTS_MOTION_LIBRARY","FIGMA_DESIGN_SYSTEM_LIBRARY","COPYWRITING_LIBRARY","MONEY_OFFER_LIBRARY","COMMERCIAL_CRAFT_LIBRARY","STORY_PSYCHOLOGY_LAYER"],
 "offer":["MONEY_OFFER_LIBRARY","SALES_OUTREACH_LIBRARY","COPYWRITING_LIBRARY","STORY_PSYCHOLOGY_LAYER"],
 "deck":["FIGMA_DESIGN_SYSTEM_LIBRARY","COPYWRITING_LIBRARY","MONEY_OFFER_LIBRARY","STORY_PSYCHOLOGY_LAYER"],
 "social_rollout":["SOCIAL_DISTRIBUTION_LIBRARY","COPYWRITING_LIBRARY","PREMIERE_EDITING_LIBRARY","AFTER_EFFECTS_MOTION_LIBRARY","COMMERCIAL_CRAFT_LIBRARY","STORY_PSYCHOLOGY_LAYER"],
 "photo_post":["PHOTO_DIRECTION_LIBRARY","ADOBE_OPERATOR_LIBRARY","SOCIAL_DISTRIBUTION_LIBRARY","COPYWRITING_LIBRARY","COMMERCIAL_CRAFT_LIBRARY","STORY_PSYCHOLOGY_LAYER"],
 ...
 "brand_ip_system":["FIGMA_DESIGN_SYSTEM_LIBRARY","ADOBE_OPERATOR_LIBRARY","MONEY_OFFER_LIBRARY","COPYWRITING_LIBRARY","HIGGSFIELD_OPERATOR_LIBRARY","STORY_PSYCHOLOGY_LAYER"],
 "still_range":["HIGGSFIELD_OPERATOR_LIBRARY","ADOBE_OPERATOR_LIBRARY","PHOTO_DIRECTION_LIBRARY","BLENDER_PRODUCTION_LIBRARY","COMMERCIAL_CRAFT_LIBRARY","STORY_PSYCHOLOGY_LAYER"],
}
...
def cmd_load(ptype):
    ...
    for lib in libs:
        base=lib.split("  ")[0]
        if base=="STORY_PSYCHOLOGY_LAYER":
            import json as _j
            spf=os.path.join(HERE,"..","story_psychology_layer","STORY_INTELLIGENCE_CARDS.json")
            n=len(_j.load(open(spf))["cards"]) if os.path.exists(spf) else 0
            print(f"  - {base:32s} {n:4d} cards  (STORY+PSYCH; load via os_story_gate.py load %s; gate via os_story_gate.py gate)" % ptype)
            manifest["required_libraries"].append(base); manifest["total_cards_available"]+=n
            continue
        if base=="COMMERCIAL_CRAFT_LIBRARY":
            import json as _j
            ccf=os.path.join(HERE,"..","OS_COMMERCIAL_CRAFT_CARDS.json")
            n=len(_j.load(open(ccf))) if os.path.exists(ccf) else 0
            print(f"  - {base:32s} {n:4d} cards  (reference-craft; gate via os_reference_gate.py)")
            manifest["required_libraries"].append(base); manifest["total_cards_available"]+=n
            continue
