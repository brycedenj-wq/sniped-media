...
TOOLS = {
    ...
    "local.gemini":      {"kind":"local","status":"ACTIVE","call":"gemini -p \"...\" --output-format json","note":"second-model critique lane (Gemini CLI 0.45.2, READ-ONLY, 0 edits verified)"},
    "local.ripgrep":     {"kind":"local","status":"ACTIVE","call":"rg","note":"fast repo search (req by gemini), 15.1.0"},
    "os.gemini_review":  {"kind":"local","status":"ACTIVE","call":"scripts/os_gemini_review.py","note":"second-model review: bundle (contact sheet) + read-only run, saves json+md"},
    "os.second_model_gate":{"kind":"local","status":"ACTIVE","call":"scripts/os_second_model_gate.py","note":"reconcile second-model notes vs evidence (accept/partial/reject)"},
    ...
    "mcp.premiere":   {"kind":"mcp","status":"ACTIVE","call":"node ~/premiere-pro-mcp/dist/index.js","note":"DIRECT Premiere control 269 tools; PROVEN sequence-build (create_sequence_from_clips) + FCPXML interchange export 2026-06-06 (SOLE sprint). AME video render BLOCKED (export_sequence false-acks, add_to_render_queue illegal-param) -> route ffmpeg/aerender. create_sequence needs preset; create_bars_and_tone/export_frame buggy on 26.2.2. gate os_premiere_compliance_gate","approval":"edit-export"},
    "mcp.after_effects":{"kind":"mcp","status":"ACTIVE","call":"node ~/after-effects-mcp/build/index.js","note":"AE control; AUTHORING PROVEN 2026-06-06 (SOLE sprint): createComposition + createTextLayer + listCompositions read-back round-trip live via MCP Bridge Auto panel. getLayerInfo needs active comp (use listCompositions). aerender CLI = render route. in-AE panel must stay open","approval":"comp-render"},
    ...
    # ---- naming operating layer (STANDING capability 2026-06-06) ----
    "os.naming_engine":{"kind":"local","status":"ACTIVE","call":"scripts/os_naming_engine.py","note":"generate brand-name candidates across 8 lanes (--avoid/--clean); root bank + sound-symbolism; 1284 capacity. STANDING: any brand/world/offer/package/identity name routes here FIRST"},
    "os.name_gate":{"kind":"local","status":"ACTIVE","call":"scripts/os_name_gate.py","note":"incumbent-aware 14-criteria scorer -> risk TIER (SAFE-ish/COUNSEL/DO-NOT-USE/CODENAME) + next legal+domain checks; legacy whois subcmd. Pair with Vercel domain MCP + web incumbent check. Never claims legal clearance"},
    # ---- reference ingestion / commercial craft layer (STANDING 2026-06-06) ----
    "os.reference_ingest":{"kind":"local","status":"ACTIVE","call":"scripts/os_reference_ingest.py","note":"link/file -> reference package: yt-dlp (player_client=android bypasses 403) download, ffprobe, ffmpeg scene-cut shot map + pacing, keyframes, captions or audio for STT, TEARDOWN scaffold. PROVEN on a real YouTube link 2026-06-06. Internal study only"},
    "os.commercial_card":{"kind":"local","status":"ACTIVE","call":"scripts/os_commercial_card.py","note":"manage OS_COMMERCIAL_CRAFT_CARDS.json (add/list/show/render-md). Turns a teardown into repeatable editing/copy/design/audio moves (8-field cards). 7 seeded"},
    "os.reference_gate":{"kind":"local","status":"ACTIVE","call":"scripts/os_reference_gate.py","note":"format-AWARE edit gate (V2): classify format first via --type (comedy/product_spot/beauty_fashion/luxury_manifesto/social_teaser/bts_personality/tutorial). Computes shots/ASL/contrast/uniformity; named verdicts TOO_SLOW(only if unmotivated/repetitive/no-payoff)/TOO_FAST/TOO_REPETITIVE/NO_PAYOFF/LOW_SHOT_VARIATION/AUDIO. + 12-axis scorecard + profiles. Heinz comedy PASS, SOLE luxury FAIL(no payoff)"},
    "os.build_reel_from_footage":{"kind":"local","status":"ACTIVE","call":"scripts/os_build_reel_from_footage.py","note":"footage dir -> selects + shot-classify + edit_plan/EDL -> ffmpeg rough cut (16:9 + 9:16) -> auto-runs os_reference_gate. ffmpeg=assembly spine; Premiere/AE(FCPXML)/Adobe(grade)/Figma(title) = optional finish route. PROVEN end-to-end smoke test"},
    # ---- voice + automation MCP, registered + verified read-only 2026-06-06 ----
    "mcp.elevenlabs": {"kind":"mcp","status":"ACTIVE","call":"mcp__ElevenLabs__*","note":"VO/TTS/SFX/voice-clone/ConvAI/outbound-call. PROVEN read-only 2026-06-06: check_subscription (tier FREE, 0/10000 chars, 3 voice slots), list_models (10 incl eleven_v3 + multilingual_v2 + flash_v2_5), search_voices (premade lib). Capital ElevenLabs = absolute-path uvx; lowercase duplicate REMOVED (same key). free-tier TTS VO PROVEN 2026-06-06 (SOLE sprint: text_to_speech Roger/eleven_multilingual_v2 -> mp3, ~32 chars). GENERATION/CALLS otherwise HELD: no SFX/agent/outbound without explicit go. Every generate burns the 10k char/mo cap; cloning + agents + v3 tags need paid/verify. Route os_voice_agent_router, gate os_audio_stack_gate","approval":"generate-call"},
    "mcp.n8n":        {"kind":"mcp","status":"ACTIVE","call":"mcp__n8n-mcp__*","note":"workflow node/template knowledge + (with N8N_API_URL) live workflow CRUD. PROVEN read-only 2026-06-06: tools_documentation overview (21 tools: search_nodes/get_node/validate_node/validate_workflow/get_template/search_templates + 15 n8n_* API tools). Doc/validate ACTIVE; LIVE workflow execution HELD (n8n_* API tools need N8N_API_URL config + explicit go)","approval":"live-workflow"},
}
