#!/usr/bin/env python3
"""
Extract the 6 CORE BIOGRAPHY_FOUNDER_MEDIA sources from
raw/03_TIER_2_CANON_BOOKS/memoirs_biographies/ into
01_KNOWLEDGE_BASE/batches/biography_founder_media_extracted/.

4 epub via stdlib zipfile + spine-ordered HTML-strip · 2 pdf via pdftotext -layout.
Keyword-substring matching on filenames. No OCR. No new deps.
...
OUT = ROOT / "01_KNOWLEDGE_BASE" / "batches" / "biography_founder_media_extracted"
OUT.mkdir(parents=True, exist_ok=True)

# (output_name, keyword, kind)
TARGETS = [
    ("dv_vreeland.txt", "Vreeland, Diana - D.V.", "epub"),
    ("no_filter_instagram_frier.txt", "No Filter", "epub"),
    ("losing_my_virginity_branson.txt", "Losing My Virginity", "epub"),
    ("grinding_it_out_kroc.txt", "Grinding It Out", "epub"),
    ("that_will_never_work_randolph.txt", "That Will Never Work", "epub"),
    ("made_in_japan_morita.txt", "Made in Japan", "pdf"),
]
